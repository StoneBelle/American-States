...
while len(score.valid_answers) < 50:
    answer = wn.textinput(title="Guess the States", prompt=user_prompt).title().strip()
    # If user exits from game early, store all the states that have not been named in a new CSV file.
    if answer == "Exit":
        missing_states = [state for state in all_states if state not in score.valid_answers]
